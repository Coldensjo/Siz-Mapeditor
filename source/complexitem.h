...
public:
	...
	void setKeyHoleNumber(const uint16_t number) {
	...
	void setDoorLevel(const uint16_t level) {
		doorLevel = level;
	}

	uint16_t getQuestNumber() const {
		return questNumber;
	}
	void setQuestNumber(const uint16_t number) {
		questNumber = number;
	}

	uint16_t getQuestValue() const {
		return questValue;
	}
	void setQuestValue(const uint16_t value) {
		questValue = value;
	}
...
public:
	...
	void setKeyNumber(uint16_t number) {
		keyNumber = number;
	}
