...
std::string swapExtension(const std::string& filename, const char* extension)
{
...
bool resolveAuxFile(const FileName& dir, std::string& auxFile, std::string& resolvedPath)
{
	if (auxFile.empty()) {
		return false;
	}

	FileName directPath(wxstr(auxFile));
	if (directPath.IsAbsolute() && directPath.FileExists()) {
		resolvedPath = auxFile;
		return true;
	}

	const std::string basePath = (const char*)(dir.GetPath(wxPATH_GET_SEPARATOR | wxPATH_GET_VOLUME).mb_str(wxConvUTF8));
	resolvedPath = basePath + auxFile;

	FileName filename(wxstr(resolvedPath));
	if (filename.FileExists()) {
		return true;
	}

	if (hasExtension(auxFile, ".xml")) {
		const std::string alternateFile = swapExtension(auxFile, ".toml");
		const std::string alternatePath = basePath + alternateFile;
		if (FileName(wxstr(alternatePath)).FileExists()) {
			auxFile = alternateFile;
			resolvedPath = alternatePath;
			return true;
		}
	} else if (hasExtension(auxFile, ".toml")) {
		const std::string alternateFile = swapExtension(auxFile, ".xml");
		const std::string alternatePath = basePath + alternateFile;
		if (FileName(wxstr(alternatePath)).FileExists()) {
			auxFile = alternateFile;
			resolvedPath = alternatePath;
...
bool writeTomlFile(const std::string& path, const toml::table& rootTable)
{
...
bool Item::readItemAttribute_OTBM(const IOMap& maphandle, OTBM_ItemAttribute attr, BinaryNode* stream) {
	switch (attr) {
		...
		case OTBM_ATTR_KEYNUMBER:
			return stream->skip(2);
		case OTBM_ATTR_HOUSEDOORID:
			return stream->skip(1);
		case OTBM_ATTR_TELE_DEST:
			return stream->skip(5);
		case OTBM_ATTR_KEYHOLENUMBER:
			return stream->skip(2);
		case OTBM_ATTR_DOORLEVEL:
			return stream->skip(2);
		case OTBM_ATTR_DOORQUESTNUMBER:
			return stream->skip(2);
		case OTBM_ATTR_DOORQUESTVALUE:
			return stream->skip(2);
...
bool IOMapOTBM::loadMap(Map& map, const FileName& filename) {
	...
	if (filename.GetExt() == "otgz") {
		...
		bool house_buffer_is_toml = false;
		bool spawn_buffer_is_toml = false;
		...
		while (archive_read_next_header(a.get(), &entry) == ARCHIVE_OK) {
			...
			} else if (entryName == "world/houses.toml") {
				house_buffer_size = archive_entry_size(entry);
				house_buffer.reset(new uint8_t[house_buffer_size]);
				house_buffer_is_toml = true;
			...
			} else if (entryName == "world/spawns.toml") {
				spawn_buffer_size = archive_entry_size(entry);
				spawn_buffer.reset(new uint8_t[spawn_buffer_size]);
				spawn_buffer_is_toml = true;
		...
		if (house_buffer.get() && house_buffer_size > 0) {
			if (house_buffer_is_toml) {
				toml::table rootTable;
				const std::string_view bufferView(reinterpret_cast<const char*>(house_buffer.get()), house_buffer_size);
				TomlNode doc = TomlNode::fromBuffer(bufferView, rootTable);
				if (doc) {
					if (!loadHouses(map, doc)) {
						warning("Failed to load houses.");
					}
				} else {
					warning("Failed to load houses due to TOML parse error.");
				}
			} else {
		...
		if (spawn_buffer.get() && spawn_buffer_size > 0) {
			if (spawn_buffer_is_toml) {
				toml::table rootTable;
				const std::string_view bufferView(reinterpret_cast<const char*>(spawn_buffer.get()), spawn_buffer_size);
				TomlNode doc = TomlNode::fromBuffer(bufferView, rootTable);
				if (doc) {
					...
					warning("Failed to load spawns due to TOML parse error.");
				}
			} else {
				pugi::xml_document doc;
	...
	if (!loadAuxiliaryHouses(map, filename)) {
		warning("Failed to load houses.");
	}
	if (!loadAuxiliarySpawns(map, filename)) {
		warning("Failed to load spawns.");
	}
	if (!loadAuxiliaryComments(map, filename)) {
		warning("Failed to load map comments.");
	}
	return true;
}

bool IOMapOTBM::loadAuxiliaryHouses(Map& map, const FileName& mapFile) {
	IOMapOTBM loader(map.getVersion());
	const std::string mapBaseName = nstr(mapFile.GetName());
	const std::string fallbackToml = mapBaseName + "-house.toml";
	const std::string fallbackXml = mapBaseName + "-house.xml";

	if (map.housefile.empty()) {
		map.housefile = fallbackToml;
	}

	if (loader.loadHouses(map, mapFile)) {
		return true;
	}

	const std::string attempted = map.housefile;
	if (attempted != fallbackToml) {
		map.housefile = fallbackToml;
		if (loader.loadHouses(map, mapFile)) {
			return true;
		}
	}
	if (attempted != fallbackXml && map.housefile != fallbackXml) {
		map.housefile = fallbackXml;
		if (loader.loadHouses(map, mapFile)) {
			return true;
		}
	}

	map.housefile = attempted;
	return false;
}

bool IOMapOTBM::loadAuxiliarySpawns(Map& map, const FileName& mapFile) {
	IOMapOTBM loader(map.getVersion());
	const std::string mapBaseName = nstr(mapFile.GetName());
	const std::string fallbackToml = mapBaseName + "-spawn.toml";
	const std::string fallbackXml = mapBaseName + "-spawn.xml";

	if (map.spawnfile.empty()) {
		map.spawnfile = fallbackToml;
	}

	if (loader.loadSpawns(map, mapFile)) {
		return true;
	}

	const std::string attempted = map.spawnfile;
	if (attempted != fallbackToml) {
		map.spawnfile = fallbackToml;
		if (loader.loadSpawns(map, mapFile)) {
			return true;
	...
	if (attempted != fallbackXml && map.spawnfile != fallbackXml) {
		map.spawnfile = fallbackXml;
		if (loader.loadSpawns(map, mapFile)) {
			return true;
		}
	}

	map.spawnfile = attempted;
	return false;
}

bool IOMapOTBM::loadAuxiliaryComments(Map& map, const FileName& mapFile) {
	IOMapOTBM loader(map.getVersion());
	const std::string mapBaseName = nstr(mapFile.GetName());
	const std::string fallbackXml = mapBaseName + "-comments.xml";
	const std::string fallbackToml = mapBaseName + "-comments.toml";

	if (map.commentsfile.empty()) {
		map.commentsfile = fallbackXml;
	}

	if (loader.loadComments(map, mapFile)) {
		return true;
	}

	const std::string attempted = map.commentsfile;
	if (attempted != fallbackXml) {
		map.commentsfile = fallbackXml;
		if (loader.loadComments(map, mapFile)) {
			return true;
		}
	}
	if (attempted != fallbackToml && map.commentsfile != fallbackToml) {
		map.commentsfile = fallbackToml;
		if (loader.loadComments(map, mapFile)) {
			return true;
		}
	}

	map.commentsfile = attempted;
	return false;
}
...
bool IOMapOTBM::loadMap(Map& map, NodeFileReadHandle& f) {
	...
	for (BinaryNode* mapNode = mapHeaderNode->getChild(); mapNode != nullptr; mapNode = mapNode->advance()) {
		++nodes_loaded;
		if ((nodes_loaded & 0x3F) == 0 && totalSize > 0) {
			const int32_t progress = static_cast<int32_t>(100.0 * f.tell() / totalSize);
		...
		if (node_type == OTBM_TILE_AREA) {
			...
			// Waypoints are no longer supported, skip this node
...
bool IOMapOTBM::loadSpawns(Map& map, const FileName& dir) {
	std::string auxFile = map.spawnfile;
	std::string encoded_path;
	if (!resolveAuxFile(dir, auxFile, encoded_path)) {
		warnings.push_back("IOMapOTBM::loadSpawns: File not found.");
		return false;
	}

	if (auxFile != map.spawnfile) {
		map.spawnfile = auxFile;
	}

	if (hasExtension(auxFile, ".toml")) {
		toml::table rootTable;
		TomlNode doc = TomlNode::fromFile(encoded_path, rootTable);
		if (!doc) {
...
bool IOMapOTBM::loadSpawns(Map& map, pugi::xml_document& doc) {
	...
	const int defaultSpawnTime = g_settings.getInteger(Config::DEFAULT_SPAWNTIME);
	...
	for (pugi::xml_node spawnNode = node.first_child(); spawnNode; spawnNode = spawnNode.next_sibling()) {
		...
		Tile* tile = map.createTile(spawnPosition.x, spawnPosition.y, spawnPosition.z);
		if (tile->spawn) {
		...
		for (pugi::xml_node creatureNode = spawnNode.first_child(); creatureNode; creatureNode = creatureNode.next_sibling()) {
			...
			radius = std::min<int32_t>(radius, maxSpawnRadius);
		...
		if (!creatureTile) {
			creatureTile = map.createTile(creaturePosition.x, creaturePosition.y, creaturePosition.z);
		}
...
bool IOMapOTBM::loadHouses(Map& map, const FileName& dir) {
	std::string auxFile = map.housefile;
	std::string encoded_path;
	if (!resolveAuxFile(dir, auxFile, encoded_path)) {
	...
	if (auxFile != map.housefile) {
		map.housefile = auxFile;
	}
...
bool IOMapOTBM::saveMap(Map& map, const FileName& identifier) {
	...
	if (identifier.GetExt() == "otgz") {
		...
		const bool spawnIsToml = hasExtension(map.spawnfile, ".toml");
		if (spawnIsToml) {
			toml::table spawnRoot;
			if (saveSpawns(map, spawnRoot)) {
				std::ostringstream streamData;
				streamData << spawnRoot;
				const std::string tomlData = streamData.str();

				entry = archive_entry_new();
				archive_entry_set_pathname(entry, "world/spawns.toml");
				archive_entry_set_size(entry, tomlData.size());
				archive_entry_set_filetype(entry, AE_IFREG);
				archive_entry_set_perm(entry, 0644);

				archive_write_header(a, entry);
				archive_write_data(a, tomlData.data(), tomlData.size());
		...
		const bool houseIsToml = hasExtension(map.housefile, ".toml");
		if (houseIsToml) {
			toml::table houseRoot;
			if (saveHouses(map, houseRoot)) {
				std::ostringstream streamData;
				streamData << houseRoot;
				const std::string tomlData = streamData.str();

				entry = archive_entry_new();
				archive_entry_set_pathname(entry, "world/houses.toml");
				archive_entry_set_size(entry, tomlData.size());
				archive_entry_set_filetype(entry, AE_IFREG);
				archive_entry_set_perm(entry, 0644);
	...
	g_gui.SetLoadDone(99, "Saving map comments...");
	saveComments(map, identifier);
...
bool IOMapOTBM::saveSpawns(Map& map, const FileName& dir) {
	...
	if (hasExtension(map.spawnfile, ".toml")) {
		toml::table rootTable;
		if (saveSpawns(map, rootTable)) {
			return writeTomlFile(nstr(filepath), rootTable);
		}
		return false;
	}
...
bool IOMapOTBM::saveSpawns(Map& map, pugi::xml_document& doc) {
	...
	for (const auto& spawnPosition : map.spawns) {
		...
		int32_t spawnRadius = spawn->getSize();
		spawnNode.append_attribute("radius") = spawnRadius;

		for (int32_t y = -spawnRadius; y <= spawnRadius; ++y) {
			for (int32_t x = -spawnRadius; x <= spawnRadius; ++x) {
...
bool IOMapOTBM::saveHouses(Map& map, const FileName& dir) {
	...
	if (hasExtension(map.housefile, ".toml")) {
		toml::table rootTable;
		if (saveHouses(map, rootTable)) {
			return writeTomlFile(nstr(filepath), rootTable);
		}
		return false;
	}
...
bool IOMapOTBM::loadRegularSpawnTomlEntry(Map& map, TomlNode& spawnNode, int defaultSpawnTime, int maxSpawnRadius)
{
	Position spawnPosition;
	spawnPosition.x = spawnNode.attribute("centerx").as_int();
	spawnPosition.y = spawnNode.attribute("centery").as_int();
	...
	for (auto creatureNode : spawnNode.child("monster")) {
		const std::string name = creatureNode.attribute("name").as_string();
		if (name.empty()) {
			wxString err;
			err << "Bad creature position data, discarding creature at spawn " << spawnPosition.x << ":" << spawnPosition.y << ":" << spawnPosition.z << " due missing name.";
		...
		Direction direction = NORTH;
		int dir = -1;
		if (TomlAttribute directionAttribute = creatureNode.attribute("direction")) {
			dir = directionAttribute.as_int();
		}
		if (dir >= DIRECTION_FIRST && dir <= DIRECTION_LAST) {
		...
		Position creaturePosition(spawnPosition);
		if (TomlAttribute xAttribute = creatureNode.attribute("x")) {
			creaturePosition.x += xAttribute.as_int();
		}
		if (TomlAttribute yAttribute = creatureNode.attribute("y")) {
			creaturePosition.y += yAttribute.as_int();
		}
		...
		CreatureType* type = g_creatures[name];
		if (!type) {
			type = g_creatures.addMissingCreatureType(name, false);
		}
		...
		if (creatureTile->getLocation()->getSpawnCount() == 0) {
			ASSERT(creatureTile->spawn == nullptr);
			Spawn* orphanSpawn = newd Spawn(5);
			creatureTile->spawn = orphanSpawn;
			map.addSpawn(creatureTile);
		}
	}

	for (auto creatureNode : spawnNode.child("npc")) {
		const std::string name = creatureNode.attribute("name").as_string();
		if (name.empty()) {
			wxString err;
			err << "Bad creature position data, discarding creature at spawn " << spawnPosition.x << ":" << spawnPosition.y << ":" << spawnPosition.z << " due missing name.";
...
bool IOMapOTBM::loadSpawnsFromTomlEntries(Map& map, TomlNode& spawnEntries)
{
	const int defaultSpawnTime = g_settings.getInteger(Config::DEFAULT_SPAWNTIME);
	const int maxSpawnRadius = g_settings.getInteger(Config::MAX_SPAWN_RADIUS);

	for (auto spawnNode : spawnEntries) {
		if (!loadRegularSpawnTomlEntry(map, spawnNode, defaultSpawnTime, maxSpawnRadius)) {
			return false;
		}
	}
	return true;
...
bool IOMapOTBM::loadSpawns(Map& map, TomlNode& doc) {
	TomlNode spawnEntries = doc.child("spawns");
	if (spawnEntries) {
		return loadSpawnsFromTomlEntries(map, spawnEntries);
	}

	TomlNode regularEntries = doc.child("spawn");
	if (regularEntries) {
		return loadSpawnsFromTomlEntries(map, regularEntries);
	}
...
bool IOMapOTBM::loadHouses(Map& map, TomlNode& doc) {
	TomlNode housesNode = doc.child("house");
	if (!housesNode) {
		housesNode = doc.child("houses");
	}
	if (!housesNode) {
		warnings.push_back("IOMapOTBM::loadHouses: Invalid rootheader.");
		return false;
	...
	for (auto houseNode : housesNode) {
		const std::string nodeName = as_lower_str(houseNode.name());
		if (nodeName != "house" && !houseNode.attribute("houseid")) {
			continue;
		}

		House* house = nullptr;
		if (TomlAttribute houseIdAttribute = houseNode.attribute("houseid")) {
			house = map.houses.getHouse(static_cast<uint32_t>(houseIdAttribute.as_uint64()));
			if (!house) {
				break;
			}
		}

		if (TomlAttribute nameAttribute = houseNode.attribute("name")) {
			house->name = nameAttribute.as_string();
		} else {
			house->name = "House #" + std::to_string(house->getID());
		}
		...
		if (TomlAttribute rentAttribute = houseNode.attribute("rent")) {
			house->rent = rentAttribute.as_int();
		}

		if (TomlAttribute guildHallAttribute = houseNode.attribute("guildhall")) {
			house->guildhall = guildHallAttribute.as_bool();
		}

		if (TomlAttribute townIdAttribute = houseNode.attribute("townid")) {
			house->townid = static_cast<uint32_t>(townIdAttribute.as_uint64());
		} else {
			warning("House %d has no town! House was removed.", house->getID());
			map.houses.removeHouse(house);
		}
...
bool IOMapOTBM::saveSpawns(Map& map, toml::table& rootTable) {
	CreatureList creatureList;
	toml::array regularSpawnEntries;
	...
	for (const auto& spawnPosition : map.spawns) {
		...
		toml::table spawnEntry;
		spawnEntry.insert("centerx", static_cast<int64_t>(spawnPosition.x));
		spawnEntry.insert("centery", static_cast<int64_t>(spawnPosition.y));
		spawnEntry.insert("centerz", static_cast<int64_t>(spawnPosition.z));

		const int32_t spawnRadius = spawn->getSize();
		spawnEntry.insert("radius", static_cast<int64_t>(spawnRadius));

		toml::array monsterEntries;
		toml::array npcEntries;
		...
		for (int32_t y = -spawnRadius; y <= spawnRadius; ++y) {
			for (int32_t x = -spawnRadius; x <= spawnRadius; ++x) {
				...
				toml::table creatureEntry;
				creatureEntry.insert("name", creature->getName());
				creatureEntry.insert("x", static_cast<int64_t>(x));
				creatureEntry.insert("y", static_cast<int64_t>(y));
				creatureEntry.insert("z", static_cast<int64_t>(spawnPosition.z));
				creatureEntry.insert("spawntime", static_cast<int64_t>(creature->getSpawnTime()));
				if (creature->getDirection() != NORTH) {
					creatureEntry.insert("direction", static_cast<int64_t>(creature->getDirection()));
				}

				if (creature->isNpc()) {
					npcEntries.push_back(creatureEntry);
				} else {
					monsterEntries.push_back(creatureEntry);
				}
		...
		if (!monsterEntries.empty()) {
			spawnEntry.insert("monster", std::move(monsterEntries));
		}
		if (!npcEntries.empty()) {
			spawnEntry.insert("npc", std::move(npcEntries));
		}

		regularSpawnEntries.push_back(spawnEntry);
	}
	...
	if (!regularSpawnEntries.empty()) {
		rootTable.insert("spawn", std::move(regularSpawnEntries));
	}
	return true;
}

bool IOMapOTBM::saveHouses(Map& map, toml::table& rootTable) {
	toml::array houseEntries;
	for (const auto& houseEntry : map.houses) {
		const House* house = houseEntry.second;
		toml::table entry;
		entry.insert("name", house->name);
		entry.insert("houseid", static_cast<int64_t>(house->getID()));

		const Position& exitPosition = house->getExit();
		entry.insert("entryx", static_cast<int64_t>(exitPosition.x));
		entry.insert("entryy", static_cast<int64_t>(exitPosition.y));
		entry.insert("entryz", static_cast<int64_t>(exitPosition.z));
		entry.insert("rent", static_cast<int64_t>(house->rent));
		if (house->guildhall) {
			entry.insert("guildhall", true);
		}
		entry.insert("townid", static_cast<int64_t>(house->townid));
		entry.insert("size", static_cast<int64_t>(house->size()));
		houseEntries.push_back(entry);
	}

	rootTable.insert("house", std::move(houseEntries));
	return true;
}

bool IOMapOTBM::loadComments(Map& map, const FileName& dir) {
	std::string auxFile = map.commentsfile;
	std::string encoded_path;
	if (!resolveAuxFile(dir, auxFile, encoded_path)) {
		return false;
	}

	if (auxFile != map.commentsfile) {
		map.commentsfile = auxFile;
	}
	...
	if (hasExtension(auxFile, ".toml")) {
		...
		TomlNode doc = TomlNode::fromFile(encoded_path, rootTable);
		if (!doc) {
			warnings.push_back("IOMapOTBM::loadComments: File loading error.");
			return false;
		}

		std::vector<MapComment> loaded;
		uint32_t nextId = 1;
		if (TomlAttribute nextIdAttribute = doc.attribute("next_id")) {
			nextId = static_cast<uint32_t>(nextIdAttribute.as_uint64());
		}

		TomlNode commentsNode = doc.child("comment");
		if (commentsNode) {
			for (auto commentNode : commentsNode) {
				MapComment comment;
				if (TomlAttribute idAttribute = commentNode.attribute("id")) {
					comment.id = static_cast<uint32_t>(idAttribute.as_uint64());
				}
				if (TomlAttribute xAttribute = commentNode.attribute("x")) {
					comment.pos.x = xAttribute.as_int();
				}
				if (TomlAttribute yAttribute = commentNode.attribute("y")) {
					comment.pos.y = yAttribute.as_int();
				}
				if (TomlAttribute zAttribute = commentNode.attribute("z")) {
					comment.pos.z = zAttribute.as_int();
				}
				if (TomlAttribute authorAttribute = commentNode.attribute("author")) {
					comment.author = authorAttribute.as_string();
				}
				if (TomlAttribute textAttribute = commentNode.attribute("text")) {
					comment.text = textAttribute.as_string();
				}
				if (TomlAttribute createdAttribute = commentNode.attribute("created")) {
					comment.createdUnix = createdAttribute.as_int();
				}
				if (comment.text.empty()) {
					continue;
				}
				loaded.push_back(comment);
			}
		}

		map.comments.setAll(std::move(loaded), nextId);
		return true;
	}
	...
	return loadComments(map, doc);
}

bool IOMapOTBM::loadComments(Map& map, pugi::xml_document& doc) {
	pugi::xml_node rootNode = doc.child("comments");
	if (!rootNode) {
		warnings.push_back("IOMapOTBM::loadComments: Invalid root node.");
		return false;
	}

	std::vector<MapComment> loaded;
	uint32_t nextId = static_cast<uint32_t>(rootNode.attribute("next_id").as_uint(1));

	for (pugi::xml_node commentNode : rootNode.children("comment")) {
		MapComment comment;
		comment.id = commentNode.attribute("id").as_uint();
		comment.pos.x = commentNode.attribute("x").as_int();
		comment.pos.y = commentNode.attribute("y").as_int();
		comment.pos.z = commentNode.attribute("z").as_int();
		comment.author = commentNode.attribute("author").as_string();
		comment.createdUnix = commentNode.attribute("created").as_int();
		comment.text = commentNode.text().get();
		if (comment.text.empty()) {
			comment.text = commentNode.attribute("text").as_string();
		}
		if (comment.text.empty()) {
			continue;
		}
...
bool IOMapOTBM::saveComments(Map& map, const FileName& dir) {
	if (map.commentsfile.empty() || !hasExtension(map.commentsfile, ".xml")) {
		map.commentsfile = nstr(dir.GetName()) + "-comments.xml";
	}

	pugi::xml_document doc;
	if (!saveComments(map, doc)) {
		return false;
	}

	wxString filepath = dir.GetPath(wxPATH_GET_SEPARATOR | wxPATH_GET_VOLUME);
	filepath += wxString(map.commentsfile.c_str(), wxConvUTF8);
	return doc.save_file(filepath.wc_str(), "\t", pugi::format_default, pugi::encoding_utf8);
}

bool IOMapOTBM::saveComments(Map& map, pugi::xml_document& doc) {
	pugi::xml_node decl = doc.prepend_child(pugi::node_declaration);
	if (!decl) {
		return false;
	}
	...
	pugi::xml_node rootNode = doc.append_child("comments");
	rootNode.append_attribute("next_id") = static_cast<unsigned int>(map.comments.getNextId());

	for (const MapComment& comment : map.comments.all()) {
		pugi::xml_node commentNode = rootNode.append_child("comment");
		commentNode.append_attribute("id") = static_cast<unsigned int>(comment.id);
		commentNode.append_attribute("x") = comment.pos.x;
		commentNode.append_attribute("y") = comment.pos.y;
		commentNode.append_attribute("z") = comment.pos.z;
		commentNode.append_attribute("author") = comment.author.c_str();
		commentNode.append_attribute("created") = static_cast<int>(comment.createdUnix);
		commentNode.text().set(comment.text.c_str());
	}
