...
uint32_t Settings::getRevision() const {
	return revision;
}
...
#define String(key, dflt)                             \
	...
	Int(TRANSPARENT_ITEMS_ON_VOID, 0);
	Int(SHOW_ALL_FLOORS, 1);
	Int(SHOW_ALL_FLOORS_UNDERGROUND, 1);
	...
	Int(SHOW_HUD, 1);
	Int(SHOW_AUTOBORDER_INDICATOR, 1);
	...
	Int(SHOW_INVISIBLE_ITEMS, 1);
	Int(SHOW_CREATURE_SPAWN_TIME, 0);
	Int(SHOW_TOOLTIP_KEY_NUMBER, 1);
	Int(SHOW_TOOLTIP_DOOR_PROPERTIES, 1);
	...
	Int(IGNORE_SAVE_PROMPT, 0);
	Int(AUTO_SAVE_ON_CLOSE, 1);
	Int(OPEN_MAP_ON_STARTUP, 0);
	String(OPEN_MAP_ON_STARTUP_PATH, "");
	...
	Int(CURSOR_CROSSHAIR_RED, 255);
	Int(CURSOR_CROSSHAIR_GREEN, 255);
	Int(CURSOR_CROSSHAIR_BLUE, 255);
	Int(VIEWPORT_BACKGROUND_RED, 0);
	Int(VIEWPORT_BACKGROUND_GREEN, 0);
	Int(VIEWPORT_BACKGROUND_BLUE, 0);

	section("UI");
	Int(USE_LARGE_CONTAINER_ICONS, 1);
	Int(USE_LARGE_CHOOSE_ITEM_ICONS, 1);
	Int(CONTAINER_FIND_DEFAULT_NAMES, 0);
	String(CONTAINER_FAVORITE_ITEMS, "");
	...
	Int(LIVE_CURSOR_BLUE, 0);
	Int(LIVE_CURSOR_ALPHA, 200);
...
void Settings::load() {
	wxConfigBase* conf;
	// Get executable directory (mapeditor root folder)
	FileName exec_path;
	try {
		exec_path = dynamic_cast<wxStandardPaths&>(wxStandardPaths::Get()).GetExecutablePath();
	...
	wxString exec_dir = exec_path.GetPath(wxPATH_GET_VOLUME | wxPATH_GET_SEPARATOR);
	
	// Construct path to editor.cfg in mapeditor root folder
	FileName filename;
	filename.Assign(exec_dir, "editor.cfg");
