...
public:
	...
	static constexpr int BRUSH_SIZE_2X2 = -1;

	BrushShape GetBrushShape() const;
	int GetBrushSize() const; // Returns the current brush radius (non-negative)
	int GetBrushSizeSelectionValue() const; // Returns the value used when selecting brush size (may be BRUSH_SIZE_2X2)
	bool IsBrushEvenSize() const;
	int GetSquareBrushMinOffset() const;
	int GetSquareBrushMaxOffset() const;
	int GetSquareBrushOuterMinOffset() const;
	int GetSquareBrushOuterMaxOffset() const;
	bool IsSingleTileBrush() const;
	...
	// Live collaboration: closes the live editor tab(s) associated with the
	// given socket (used when a connection drops or is kicked).
	void CloseLiveEditors(LiveSocket* sender);
	// When true, the GUI runs without a main window (standalone server).
	// Dialogs, progress bars and view refreshes become no-ops.
