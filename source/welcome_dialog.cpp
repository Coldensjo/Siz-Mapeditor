...
WelcomeDialog::WelcomeDialog(const wxString& title_text, const wxString& version_text, const wxSize& size, const wxBitmap& rme_logo, const std::vector<wxString>& recent_files) : wxDialog(nullptr, wxID_ANY, "", wxDefaultPosition, size) {
	Centre();
	const wxColour base_colour = ThemeManager::Get().GetPalette().surface;
...
WelcomeDialogPanel::WelcomeDialogPanel(WelcomeDialog* dialog, const wxSize& size, const wxString& title_text, const wxString& version_text, const wxColour& base_colour, const wxBitmap& rme_logo, const std::vector<wxString>& recent_files) : wxPanel(dialog),
																																								m_rme_logo(rme_logo),
																																								m_title_text(title_text),
																																								m_version_text(version_text),
																																								m_text_colour(ThemeManager::Get().GetPalette().text),
	...
	recent_maps_panel->SetBackgroundColour(ThemeManager::Get().GetPalette().window);

	wxSize button_size = FROM_DIP(this, wxSize(150, 35));
	const wxColour button_base_colour = ThemeManager::Get().GetPalette().control;
	...
	auto* live_connect_button = newd WelcomeDialogButton(this, wxDefaultPosition, button_size, button_base_colour, "Join Live");
	live_connect_button->SetAction(WELCOME_LIVE_CONNECT);
	live_connect_button->Bind(wxEVT_LEFT_UP, &WelcomeDialog::OnButtonClicked, dialog);
	...
	buttons_sizer->Add(live_connect_button, 0, wxALIGN_CENTER | wxTOP, FROM_DIP(this, 10));
...
void WelcomeDialogPanel::OnPaint(const wxPaintEvent& event) {
	...
	dc.SetTextForeground(ThemeManager::Get().GetPalette().mutedText);
	dc.DrawText(m_version_text, wxPoint(header_point.x - version_size.x / 2, header_point.y + header_size.y + 10));
}

WelcomeDialogButton::WelcomeDialogButton(wxWindow* parent, const wxPoint& pos, const wxSize& size, const wxColour& WXUNUSED(base_colour), const wxString& text) : wxPanel(parent, wxID_ANY, pos, size),
																																								m_action(wxID_CLOSE),
																																								m_text(text),
																																								m_text_colour(ThemeManager::Get().GetPalette().text),
																																								m_background(ThemeManager::Get().GetPalette().control),
																																								m_background_hover(ThemeManager::Get().GetPalette().hover),
																																						m_is_hover(false) {
...
RecentMapsPanel::RecentMapsPanel(wxWindow* parent, WelcomeDialog* dialog, const wxColour& WXUNUSED(base_colour), const std::vector<wxString>& recent_files) : wxPanel(parent, wxID_ANY) {
	wxBoxSizer* sizer = new wxBoxSizer(wxVERTICAL);
	for (const wxString& file : recent_files) {
		auto* recent_item = newd RecentItem(this, ThemeManager::Get().GetPalette().surface, file);
...
RecentItem::RecentItem(wxWindow* parent, const wxColour& WXUNUSED(base_colour), const wxString& item_name) : wxPanel(parent, wxID_ANY),
																																									   m_text_colour(ThemeManager::Get().GetPalette().text),
																																									   m_text_colour_hover(ThemeManager::Get().GetPalette().mutedText),
