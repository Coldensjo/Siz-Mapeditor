...
BEGIN_EVENT_TABLE(FindDialogListBox, wxVListBox)
EVT_CHAR(FindDialogListBox::OnChar)
END_EVENT_TABLE()
...
void FindDialogListBox::SelectAll() {
	// No-op for single-selection listbox to prevent assertion
}

void FindDialogListBox::OnChar(wxKeyEvent& event) {
...
void FindDialogListBox::SelectBrush(Brush* brush) {
	if (!brush || no_matches || cleared) {
		return;
	}
...
SortableListBox::SortableListBox(wxWindow* parent, wxWindowID id, const wxPoint& pos, const wxSize& size) : wxListBox(parent, id, pos, size, 0, nullptr, wxLB_SINGLE | wxLB_NEEDED_SB) { }
...
// WarningDialog implementation
WarningDialog::WarningDialog(wxWindow* parent, wxString title, const wxArrayString& items) :
	...
	wxSizer* stdsizer = newd wxBoxSizer(wxHORIZONTAL);
	stdsizer->Add(newd wxButton(this, LIST_DIALOG_COPY_BUTTON, "Copy"), wxSizerFlags(1).Center());
	stdsizer->Add(newd wxButton(this, wxID_OK, "OK"), wxSizerFlags(1).Center());
...
WarningDialog::~WarningDialog() = default;

void WarningDialog::OnClickCopy(wxCommandEvent&) {
	if (warning_items.empty()) {
		return;
	}

	wxString text;
	for (size_t i = 0; i < warning_items.GetCount(); ++i) {
		text += warning_items[i];
		if (i < warning_items.GetCount() - 1) {
			text += "\n";
		}
	}
...
EVT_BUTTON(LIST_DIALOG_COPY_BUTTON, WarningDialog::OnClickCopy)
...
BEGIN_EVENT_TABLE(LiveConnectWindow, wxDialog)
EVT_BUTTON(wxID_OK, LiveConnectWindow::OnClickOK)
EVT_BUTTON(wxID_CANCEL, LiveConnectWindow::OnClickCancel)
END_EVENT_TABLE()
...
LiveConnectWindow::LiveConnectWindow(wxWindow* parent) :
	...
	grid->Add(newd wxStaticText(this, wxID_ANY, "Host:"), 0, wxALIGN_CENTER_VERTICAL);
	host_ctrl = newd wxTextCtrl(this, wxID_ANY, wxstr(g_settings.getString(Config::LIVE_HOST)));
	grid->Add(host_ctrl, 1, wxEXPAND);

	grid->Add(newd wxStaticText(this, wxID_ANY, "Port:"), 0, wxALIGN_CENTER_VERTICAL);
	port_ctrl = newd wxSpinCtrl(this, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize, wxSP_ARROW_KEYS, 1, 65535, g_settings.getInteger(Config::LIVE_PORT));
	grid->Add(port_ctrl, 1, wxEXPAND);

	grid->Add(newd wxStaticText(this, wxID_ANY, "Display Name:"), 0, wxALIGN_CENTER_VERTICAL);
	username_ctrl = newd wxTextCtrl(this, wxID_ANY, wxstr(g_settings.getString(Config::LIVE_USERNAME)));
	grid->Add(username_ctrl, 1, wxEXPAND);

	grid->Add(newd wxStaticText(this, wxID_ANY, "Password:"), 0, wxALIGN_CENTER_VERTICAL);
...
wxString LiveConnectWindow::GetHost() const {
	return host_ctrl->GetValue().Trim(true).Trim(false);
}

uint16_t LiveConnectWindow::GetPort() const {
	return static_cast<uint16_t>(port_ctrl->GetValue());
}

wxString LiveConnectWindow::GetUsername() const {
	return username_ctrl->GetValue().Trim(true).Trim(false);
}
...
void LiveConnectWindow::OnClickCancel(wxCommandEvent& WXUNUSED(event)) {
