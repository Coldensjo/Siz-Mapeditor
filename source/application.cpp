...
bool Application::OnInit() {
	...
	if (m_file_to_open == wxEmptyString) {
		std::string startup_map = g_settings.getString(Config::OPEN_MAP_ON_STARTUP_PATH);
		if (g_settings.getInteger(Config::OPEN_MAP_ON_STARTUP) == 1 && !startup_map.empty()) {
			m_file_to_open = wxstr(startup_map);
		}
	}
	...
	if (g_settings.getInteger(Config::WELCOME_DIALOG) == 1 && m_file_to_open == wxEmptyString && !startup_map_enabled) {
...
bool MainFrame::DoQuerySave(bool doclose) {
	...
	if (g_gui.ShouldSave()) {
		...
		} else if (!g_settings.getBoolean(Config::IGNORE_SAVE_PROMPT)) {
			long ret = g_gui.PopupDialog(
				"Save changes",
