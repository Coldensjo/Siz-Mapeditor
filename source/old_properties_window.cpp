...
OldPropertiesWindow::OldPropertiesWindow(wxWindow* win_parent, const Map* map, const Tile* tile_parent, Item* item, wxPoint pos) :
	...
	keyholenumber_field(nullptr),
	doorlevel_field(nullptr),
	doorquestnumber_field(nullptr),
	doorquestvalue_field(nullptr),
	keynumber_field(nullptr) {
	ASSERT(edit_item);
	...
	if (Container* container = dynamic_cast<Container*>(edit_item)) {
		...
		// Adjust grid sizer to reduce vertical gaps
		wxFlexGridSizer* subsizer = newd wxFlexGridSizer(2, 10, 2); // Reduce vertical gap
		subsizer->AddGrowableCol(1);
		...
		// Adjust text field size to reduce height
		wxSizer* textsizer = newd wxBoxSizer(wxVERTICAL);
		textsizer->Add(newd wxStaticText(this, wxID_ANY, "Text"), wxSizerFlags().Center());
		text_field = newd wxTextCtrl(this, wxID_ANY, wxstr(item->getText()), wxDefaultPosition, wxSize(550, 450), wxTE_MULTILINE); // Adjust height
		textsizer->Add(text_field, wxSizerFlags().Expand());

		// Add textsizer with no extra proportion
		boxsizer->Add(textsizer, wxSizerFlags().Expand());

		// Reduce padding around the whole layout
		topsizer->Add(boxsizer, wxSizerFlags(0).Expand().Border(wxALL, 5)); // Reduce border
		...
		*/
		...
		keyholenumber_field->SetToolTip(i2ws(door->getKeyHoleNumber()));
		...
		doorlevel_field->SetToolTip(i2ws(door->getDoorLevel()));
		...
		doorquestnumber_field->SetToolTip(i2ws(door->getQuestNumber()));
		...
		doorquestvalue_field->SetToolTip(i2ws(door->getQuestValue()));
