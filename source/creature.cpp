...
Creature::Creature(CreatureType* ctype) : direction(NORTH), spawntime(0), radius(50), amount(1), saved(false), selected(false) {
	if (ctype) {
		type_name = ctype->name;
	}
}

Creature::Creature(std::string ctype_name) : type_name(ctype_name), direction(NORTH), spawntime(0), radius(50), amount(1), saved(false), selected(false) {
	////
}
