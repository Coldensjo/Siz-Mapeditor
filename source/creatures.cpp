...
CreatureType* CreatureType::loadFromOTXML(const FileName& filename, pugi::xml_document& doc, wxArrayString& warnings) {
	...
	// Extract species attribute for monsters (used as category)
	// If species attribute is missing, species remains empty and monster will be put in "Others" category
	if (!isNpc && (attribute = node.attribute("species"))) {
		std::string species = attribute.as_string();
		// Capitalize first letter for display
		if (!species.empty() && species[0] >= 'a' && species[0] <= 'z') {
			species[0] = species[0] - ('a' - 'A');
		}
		ct->species = species;
	}
...
bool CreatureDatabase::loadFromDirectory(const FileName& directory, bool expectNpc, wxString& error, wxArrayString& warnings) {
	wxString basePath = directory.GetFullPath();
	if (basePath.empty()) {
		error = expectNpc ? "NPC directory is not configured." : "Monster directory is not configured.";
		return false;
	}

	if (basePath.Last() != '/' && basePath.Last() != '\\') {
		basePath.Append(FileName::GetPathSeparator());
	}
	...
	wxArrayString files;
	size_t fileCount = wxDir::GetAllFiles(basePath, &files, "*.xml", wxDIR_FILES | wxDIR_DIRS);
	if (fileCount == 0) {
		warnings.push_back("No creature XML files found in directory \"" + basePath + "\".");
		return true;
	}

	for (size_t index = 0; index < files.size(); ++index) {
		FileName creatureFile(files[index]);

		pugi::xml_document doc;
		pugi::xml_parse_result result = doc.load_file(creatureFile.GetFullPath().mb_str());
		if (!result) {
			warnings.push_back("Couldn't open file \"" + creatureFile.GetFullName() + "\", invalid format?");
			continue;
		}

		pugi::xml_node rootNode = doc.first_child();
		std::string rootName = as_lower_str(rootNode.name());
		if (rootName != "monster" && rootName != "npc") {
			continue;
		}

		if (rootNode.attribute("file")) {
			FileName referencedFile(wxString(rootNode.attribute("file").as_string(), wxConvUTF8));
			if (!referencedFile.IsAbsolute()) {
				referencedFile.MakeAbsolute(creatureFile.GetPath());
			}
			if (!referencedFile.FileExists()) {
				warnings.push_back("File \"" + creatureFile.GetFullName() + "\" references missing creature file \"" + referencedFile.GetFullName() + "\".");
				continue;
			}

			pugi::xml_document referencedDoc;
			pugi::xml_parse_result referencedResult = referencedDoc.load_file(referencedFile.GetFullPath().mb_str());
			if (!referencedResult) {
				warnings.push_back("Couldn't open referenced creature file \"" + referencedFile.GetFullName() + "\", invalid format?");
				continue;
			}

			CreatureType* referencedType = CreatureType::loadFromOTXML(referencedFile, referencedDoc, warnings);
			if (!referencedType) {
				continue;
			}

			if (referencedType->isNpc != expectNpc) {
				wxString expectedType = expectNpc ? "NPC" : "monster";
				wxString actualType = referencedType->isNpc ? "NPC" : "monster";
				warnings.push_back("File \"" + creatureFile.GetFullName() + "\" references a " + actualType + " but was loaded from the " + expectedType + " directory.");
			}

			referencedType->standard = true;

			CreatureType* existing = (*this)[referencedType->name];
			if (existing) {
				*existing = *referencedType;
				delete referencedType;
				continue;
			}

			creature_map[as_lower_str(referencedType->name)] = referencedType;
			continue;
		}

		CreatureType* creatureType = CreatureType::loadFromOTXML(creatureFile, doc, warnings);
		if (!creatureType) {
			continue;
		}

		if (creatureType->isNpc != expectNpc) {
			wxString expectedType = expectNpc ? "NPC" : "monster";
			wxString actualType = creatureType->isNpc ? "NPC" : "monster";
			warnings.push_back("File \"" + creatureFile.GetFullName() + "\" defines a " + actualType + " but was loaded from the " + expectedType + " directory.");
		}

		creatureType->standard = true;
		CreatureType* existing = (*this)[creatureType->name];
		if (existing) {
				...
				if (current) {
					*current = *creatureType;
					...
					if (creatureType->isNpc) {
						...
						// For monsters, use species-based tilesets
						std::string tileset_name = creatureType->species.empty() ? "Others" : creatureType->species;
						auto it = g_materials.tilesets.find(tileset_name);
						if (it != g_materials.tilesets.end()) {
							tileSet = it->second;
						} else {
							// Create new species-based tileset
							tileSet = newd Tileset(g_brushes, tileset_name);
							g_materials.tilesets[tileset_name] = tileSet;
						}
					}

					CreatureBrush* brush = newd CreatureBrush(creatureType);
					g_brushes.addBrush(brush);
					creatureType->brush = brush;
