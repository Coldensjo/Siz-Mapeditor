...
public:
	...
	const MapComments& getComments() const {
		return comments;
	}
	MapComments& getComments() {
		return comments;
	}
	...
	void setCommentsFilename(const std::string& new_commentsfile);
...
protected:
	...
	std::string commentsfile; // Sticky notes / map comments sidecar
...
template <typename ForeachType>
inline void foreach_CreatureOnMap(Map& map, ForeachType& foreach, bool selectedTiles) {
	MapIterator tileiter = map.begin();
	MapIterator end = map.end();
	long long done = 0;
	...
	while (tileiter != end) {
		...
		if (tile->creature) {
			foreach (map, tile, tile->creature, done)
				;
		}
		++tileiter;
	}
