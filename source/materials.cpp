...
void Materials::createOtherTileset() {
	...
	// Clear all existing species-based tilesets (except Others and NPCs which are handled above)
	for (TilesetContainer::iterator iter = tilesets.begin(); iter != tilesets.end(); ++iter) {
		if (iter->first != "Others" && iter->first != "NPCs") {
			iter->second->getCategory(TILESET_CREATURE)->brushlist.clear();
		}
	}
	
	// Map to store species-based tilesets for monsters
	std::map<std::string, Tileset*> species_tilesets;
	...
	for (CreatureMap::iterator iter = g_creatures.begin(); iter != g_creatures.end(); ++iter) {
		...
		if (type->in_other_tileset) {
			if (type->isNpc) {
				...
				std::string tileset_name = type->species.empty() ? "Others" : type->species;
				Tileset* target_tileset = nullptr;
				
				if (tileset_name == "Others") {
					target_tileset = others;
				} else {
					auto it = species_tilesets.find(tileset_name);
					if (it != species_tilesets.end()) {
						target_tileset = it->second;
					} else {
						// Use existing tileset or create new species-based tileset
						auto tileset_it = tilesets.find(tileset_name);
						if (tileset_it != tilesets.end()) {
							target_tileset = tileset_it->second;
						} else {
							target_tileset = newd Tileset(g_brushes, tileset_name);
							tilesets[tileset_name] = target_tileset;
						}
						species_tilesets[tileset_name] = target_tileset;
					}
				}
				target_tileset->getCategory(TILESET_CREATURE)->brushlist.push_back(type->brush);
			}
