...
public:
	KeyForwardingTextCtrl(wxWindow* parent, wxWindowID id, const wxString& value = "", const wxPoint& pos = wxDefaultPosition, const wxSize& size = wxDefaultSize, long style = 0, const wxValidator& validator = wxDefaultValidator, const wxString& name = wxTextCtrlNameStr) : wxTextCtrl(parent, id, value, pos, size, style, validator, name) { }
...
public:
	WarningDialog(wxWindow* parent, wxString title, const wxArrayString& items);
 ...
 */
class LiveConnectWindow : public wxDialog {
public:
	LiveConnectWindow(wxWindow* parent);
	~LiveConnectWindow() = default;
