...
void TilesetWindow::OnClickOK(wxCommandEvent& WXUNUSED(event)) {
	if (edit_item) {
		int* categoryTypePtr = reinterpret_cast<int*>(palette_field->GetClientData(palette_field->GetSelection()));
		std::string* tilesetNamePtr = static_cast<std::string*>(tileset_field->GetClientData(tileset_field->GetSelection()));
		
		if (categoryTypePtr && tilesetNamePtr) {
			TilesetCategoryType categoryType = TilesetCategoryType(*categoryTypePtr);
			std::string tilesetName = *tilesetNamePtr;
